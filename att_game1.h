...
//update player
  for(int n=0;n<pl_no;n++)   
  {
          ...
          //collision detection with the explosion
           Exp_position **q=&explosions.create_col_flag;
            while(*q!=NULL){
                int x,y,dist2;
                x=player[n].x;
                y=player[n].y;
                dist2=pow(x-(*q)->pos.x,2)+pow(y-(*q)->pos.y,2);
                if( dist2<32*32+1){
                  textprintf_ex(screen,font,23,20,4554554,2,"HIT Player %d",n);
                  rest(2000);
                }
          ...
          */
          ...
          {
                  if (explosions.create_col_flag[n].check_flag==1){
                    int x,y,dist2;
                    x=player[n].x;
                    y=player[n].y;
                    //dist2=pow(x-explosions.create_col_flag[n].x,2)+pow(y-explosions.create_col_flag[n].y,2);
                    dist2=(x-explosions.create_col_flag[n].x)*(x-explosions.create_col_flag[n].x)
                          +(y-explosions.create_col_flag[n].y)*(y-explosions.create_col_flag[n].y);
...
//update crosshair
  if (n==0)
          {
            player[n].crosshair.pos.x=player[n].x+30;//+player[n].get_width();
            player[n].crosshair.pos.y=player[n].y;//-player[0].get_height()/2;
          }
          else
          {  
//             player->num==1;
            player[n].crosshair.pos.x=player[n].x-30;//-player[1].get_width()*.5;//-player[1].get_width()/2;
            player[n].crosshair.pos.y=player[n].y;//-player[1].get_height()/2;
          }
*/
//this is being reprogrammed in the class
...
//       player[n].crosshair.update_cros(&player[n]);


}
  for(int n=0;n<pl_no;n++)   
//     draw_sprite(dest,player[n].crosshair.get_image(),player[n].crosshair.pos.x-player[n].crosshair.get_image()->w/2,player[n].crosshair.pos.y-player[n].crosshair.get_image()->h/2);
//     draw_sprite(dest,player[n].crosshair.get_image(),player[n].crosshair.pos.x-player[n].crosshair.get_width()/2,player[n].crosshair.pos.y-player[n].crosshair.get_height()/2);

//     draw_sprite(dest,player[n].crosshair.get_image(),player[n].crosshair.x-player[n].crosshair.get_width()/2,player[n].crosshair.y-player[n].crosshair.get_height()/2);
